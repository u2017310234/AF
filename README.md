...
- 此前，金融机构普遍部署了基于NLP的早期预警系统（EWS）用于资产质量监控。传统EWS的设计遵循“宁可错杀、不可漏报”的风险厌恶原则。为了保证高召回率（Recall），系统往往设定更为敏感的触发阈值，导致精确率（Precision）显著偏低。在实务层面，这引发了严峻的“告警泛洪（Alert Flooding）”问题：大量最终被证实为虚假或不具备处置价值的告警，严重挤占一线人员的认知带宽和工作时间，导致真正的弱信号在噪声中被忽略。
- 提升预警精确率的关键路径是对信号进行二次研判。然而，这一环节高度依赖人类分析师的手工介入——通过手工查阅内部手册、历史案例与外部信源，逐步拼接证据、形成风险叙事并制定调查或处置方案。这个过程面临着流程非标准化导致审计困难、过度依赖专家经验导致结果良莠不齐、异构信源检索成本高昂的三重困境。 
- 与此同时，资深分析师往往具备“只可意会、难以言传”的默会知识：在点评项目时，资深分析师会“觉得这家公司有问题”“认为这个老板迟早出事”。这种基于感觉的没有任何证据的直观判断，有时是揭露风险的楔子。本项目是将这种直觉（输入）转换为具体化、理性化证据（输出）的尝试。
- 本项目旨在在保持高召回的前提下降低误报处置成本、提升研判输出的可执行性与可审计性、普惠化机构级分析工具。


## 项目文档

https://lcn4v0200yyo.feishu.cn/wiki/RIpmwqyDCiDaw4kZZsVcVBtLnuf?from=from_copylink
